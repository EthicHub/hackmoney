...
// TODO: permissions to set rates and other admin methods
contract Pile is  Interest {
    ...
    function incDebt(uint loan, uint currencyAmount) external { 
    ...
    // a change of the loan debt updates the rate debt and total debt
    function decDebt(uint loan, uint currencyAmount) external {
        uint rate = loanRates[loan];
        require(block.timestamp == rates[rate].lastUpdated, "rate-group-not-updated");
        uint pieAmount = toPie(rates[rate].chi, currencyAmount);
    ...
    function file(bytes32 what, uint rate, uint value) external {
        if (what == "rate") {
